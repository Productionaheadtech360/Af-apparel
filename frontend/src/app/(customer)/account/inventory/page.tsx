...
//       setIsLoading(true);
//       try {
//         const data = await accountService.getInventoryReport({ stock_level: stockLevel || undefined }) as InventoryItem[];
//         setItems(data);
...
//         <select value={stockLevel} onChange={(e) => setStockLevel(e.target.value)} className="rounded-md border border-gray-300 px-3 py-1.5 text-sm">
...
//               <tr key={`${item.variant_id}-${index}`} className={`border-b border-gray-100 last:border-0 ${item.quantity <= item.low_stock_threshold ? "bg-red-50" : ""}`}>
...
//                 <td className="px-4 py-3 text-gray-500">{[item.color, item.size].filter(Boolean).join(" / ") || "—"}</td>
//                 <td className={`px-4 py-3 text-right font-semibold ${item.quantity <= item.low_stock_threshold ? "text-red-600" : "text-gray-900"}`}>
//                   {item.quantity}
//                   {item.quantity <= item.low_stock_threshold && <span className="ml-1 text-xs font-normal text-red-400">(low)</span>}
//                 </td>
//               </tr>
//             ))}
//           </tbody>
